...
public class ShoppingCartPresent implements ShoppingCartContacts.Present {
    ...
    public void getShoppingCartGoods() {
        ...
        if (null != SharedPreferencesUtil.getId(context) && !"".equals(SharedPreferencesUtil.getId(context))) {
                ...
                switch (action) {
                    case "addnum":
                        if (success.equals("1")) {
                            ...
                            i++;
//                            SharedPreferencesUtil.setCartNum(context,SharedPreferencesUtil.getCartNum(context)+1);
...
//                            SharedPreferencesUtil.setCartNum(context,SharedPreferencesUtil.getCartNum(context)-1);
                    ...
                    case "delpro":
                        if (success.equals("1")) {
//                            int num = Integer.valueOf(list.get(position).getProductQuantity());
                            list.remove(position);
                            if (list.size() == 0) {
//                                SharedPreferencesUtil.setCartNum(context,0);
                                view.noData();
                            } else {
                                view.hadData();
//                                SharedPreferencesUtil.setCartNum(context,SharedPreferencesUtil.getCartNum(context)-num);
                                myAdapter.notifyDataSetChanged();
                            }
                            myAdapter.setPrice();
